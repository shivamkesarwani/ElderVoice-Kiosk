...
import { PhoneCall, Sliders, ShieldCheck, Globe } from './Icons';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      const timeStr = now.toLocaleTimeString(languageInfo.locale || 'en-US', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true,
      });
      const dateStr = now.toLocaleDateString(languageInfo.locale || 'en-US', {
        weekday: 'short',
        month: 'short',
        day: 'numeric',
      });
  ...
  return (
    ...
    >
      ...
      {/* Top Right: Clock, Language Dropdown, Caregiver, Accessibility, PWA Install, & SOS Button */}
      <div className="flex items-center gap-1.5 sm:gap-3 lg:gap-4 shrink-0">
        ...
        <div className="text-right flex flex-col justify-center mr-1">
          ...
          <span
            id="kiosk-clock-date"
            className="text-xs sm:text-sm lg:text-base text-[#2B2A28]/80 font-medium mt-0.5 sm:mt-1 truncate max-w-[100px] sm:max-w-none"
          >
        ...
        <LanguageDropdown onShowToast={onShowToast} />

        {/* Real-time Translator Quick Launch */}
        {onTranslatorClick && (
          <button
            type="button"
            id="btn-header-translator"
        ...
        {/* Caregiver Companion Entry Point */}
        <button
          type="button"
          id="btn-header-caregiver-access"
        ...
        >
          ...
          <span className="hidden lg:inline text-sm font-bold text-[#2B2A28]">
            {t('caregiver')}
          </span>
        </button>
        ...
        <div className="hidden sm:block">
          <button
            ...
            aria-label={`${t('emergencyCall')} - ${t('orSayEmergency')}`}
          >
            <div className="flex items-center gap-2">
              <PhoneCall className="w-5 h-5 lg:w-6 lg:h-6 text-[#FBF7EF] shrink-0 stroke-[2.5]" aria-hidden="true" />
              <span className="text-xl lg:text-2xl font-black tracking-wider leading-none">{t('sos')}</span>
            </div>
